    ...
    if (res) {
      ...
      try {
        const response = await api.delete(
          `/admin/delete/dashboard/${dashboardId}`
        );
  ...
  return (
    <div className="w-full p-2">
      {dashboards.map((value) => (
        <div
          className="w-full h-60 flex justify-center items-center overflow-hidden mb-4 shadow-lg rounded-lg cursor-pointer"
          key={value.id}
        >
          <img
            src={`${import.meta.env.VITE_API_URL.replace('/api', '')}${value.image}`}
            alt="Dashboard image"
            className="transition-transform duration-300 ease-linear hover:scale-110"
            id={value.id}
